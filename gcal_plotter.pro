...
;;xyouts,2.5,median(spec[where(lambda gt 1 and lambda lt 3)]/(10.^factor)),string(rchisq_cover,format='(D5.2)'),charsize=charsize,alignment=0.5
  ...
  if oc eq 0 then begin   ;; sigmas
     ...
     sigma = greek('sigma')+'!3';textoidl('\sigma')
     plot,[0],[0],xrange=xrange,yrange=yrange,charsize=charsize,charthick=charthick,xthick=5,ythick=5,xtitle='Wavelength (!9m!3m)',/xstyle,/ystyle,ytickn=['-3','0','3'],yticks=2,ytickv=[-3,0,3],xticklen=0.08,yminor=5,ytitle='Residual ('+sigma+')'
  ...
  endelse
  oplot,[0,10],[0,0],linestyle=2,thick=5,color=cgcolor('black')
  oplot,[0,10],[0,0],linestyle=2,thick=5,color=cgcolor('black')
  sharpcorners,thick=xthick,color=cgcolor('black')
  device,/close
